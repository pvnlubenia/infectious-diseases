...
# Uses the SEIR model framework
# See pdf "Agent-based models of spread of infectious diseases using Python" for more details
# Note: This code is a clean version: compartment assignments are different from the discussion in the pdf file
#
...
def create_agents():
    ...
    for i in range(n_agents):
        ...
        # Count number of days the agent is exposed
        agent_.days_exposed = 0
...
## Scenario 2: Somewhat limited social contact for exposed and infectious people

def move():
    
    # Exposed and infectious people do not move
    for agent_ in agents_list:
        if agent_.state != 1 and agent_.state != 2:
            agent_.x, agent_.y = rd.uniform(0, 1), rd.uniform(0, 1)
...
def infect():
    ...
    infectious_list = [agent_ for agent_ in agents_list if agent_.state == 2]
...
def monitor():
    ...
    # Add count for number of days an exposed person has been exposed
    for agent_ in agents_list:
        if agent_.state == 1:
...
def count():
    ...
    # Count exposed
    E_count = len([agent_ for agent_ in agents_list if agent_.state == 1])
    
    # Count infectious
    I_count = len([agent_ for agent_ in agents_list if agent_.state == 2])
    
    # Count recovered
    R_count = len([agent_ for agent_ in agents_list if agent_.state == 3])
...
def trend():
    ...
    ax.plot(days, E_list, color = 'magenta', label = 'Exposed')
...
n_agents = S_initial + E_initial + I_initial + R_initial
...
# Initialize daily count of exposed
E_list = []
E_list.append(E_initial)
...
# Choose a specified number of random people and make them exposed
for agent_ in rd.sample(agents_list, E_initial):
    agent_.state = 1

# From the remaining susceptible, choose a specified number of random people and make them infectious
for agent_ in rd.sample([agent_ for agent_ in agents_list if agent_.state != 1], I_initial):
    agent_.state = 2

# From the remaining susceptible, choose a specified number of random people and make them recovered
for agent_ in rd.sample([agent_ for agent_ in agents_list if agent_.state != 1 and agent_.state != 2], R_initial):
    agent_.state = 3
